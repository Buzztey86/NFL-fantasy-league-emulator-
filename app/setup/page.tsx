...
export default function SetupPage() {
  ...
  function parseImport(): { picks: DraftPick[]; error?: string } {
    ...
    for (const line of lines) {
      ...
      if (!match) return { picks: [], error: `Invalid line: "${line}" (expected e.g. "3:1")` };
  ...
  return (
    <main className="mx-auto max-w-[700px] px-6 py-10">
      ...
      <section className="card mb-6">
              ...
              >
                {PERSONAS.map((p) => (
                  <option key={p.personality} value={p.personality} disabled={usedCount(p.personality) > 1 && p.personality !== personality}>
                    {p.isHuman ? `${p.name} (${s.you})` : `${p.name} — ${p.manager}`}
                  </option>
                ))}
              </select>
            </div>
        ...
        {!isValid && <p className="text-[var(--red)] text-xs mt-3">{s.onlyOnce}</p>}
        <button
          onClick={saveOrder}
          disabled={!isValid}
      ...
      <section className="card mb-6">
        <h2 className="text-[var(--gold)] text-xs font-bold tracking-wide mb-2">{s.importTitle}</h2>
        <p className="text-xs text-[var(--text-dim)] mb-3">
          {s.importDesc} <code className="text-[var(--green)]">pick:rank</code>. {s.importExample}{" "}
          <code className="text-[var(--green)]">1:1</code>.
        </p>
        <textarea
          value={importText}
      ...
      <section className="card">
        <h2 className="text-[var(--red)] text-xs font-bold tracking-wide mb-2">{s.dangerZone}</h2>
